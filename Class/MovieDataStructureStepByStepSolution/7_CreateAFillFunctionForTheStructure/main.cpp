 ...
 * Why? At this point we can start to use a design pattern called MVC.  We
 * are going to separate the Model from the Display.  MVC means Model - View -
 * Controller.  Easily done with the data being in arrays.
 * Just copy the section of the code that reads in the data and place
 * into a function.  It is already tested out and ready to go.  Copy,
 * cut and paste.
 */
